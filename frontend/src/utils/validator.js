const validator = {
  path: {
    source: [
      v => !!v || '출발역이 없습니다.',
      v => v >= 0 || '출발역 ID는 0 이상의 양수이어야 합니다.',
      v => v.isInteger() || '출발역 ID는 정수여야 합니다.'
    ],
    target: [
      v => !!v || '도착역이 없습니다.',
      v => v >= 0 || '도착역 ID는 0 이상의 양수이어야 합니다.',
      v => v.isInteger() || '도착역 ID는 정수여야 합니다.'
    ]
  },
  departureTime: {
    dayTime: [v => v == 'am' || v == 'pm' || '오전이나 오후를 입력해 주세요.'],
    hour: [v => 1 <= v <= 12 || '시각은 1~12 사이여야 합니다.'],
    minute: [v => 0 <= v <= 60 || '분은 0~60 사이여야 합니다.']
  },
